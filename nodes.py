...
# Helper function to call LLM with settings from shared dictionary
def call_llm_with_settings(prompt, shared):
    # Get LLM settings from shared dictionary if available
    llm_settings = shared.get("llm_settings", {})
    model_provider = llm_settings.get("model_provider", "gemini")  # Default to gemini
    specific_model = llm_settings.get("specific_model")
    use_cache = llm_settings.get("use_cache", True)
    
    # Call LLM with settings
    return call_llm(
        prompt=prompt,
...
class IdentifyAbstractions(Node):
    def prep(self, shared):
        ...
        return context, file_listing_for_prompt, len(files_data), project_name, shared, language # Return language

    def exec(self, prep_res):
        context, file_listing_for_prompt, file_count, project_name, shared, language = prep_res  # Unpack project name and language
...
```"""
        response = call_llm_with_settings(prompt, shared)
...
class AnalyzeRelationships(Node):
    def prep(self, shared):
        ...
        return context, "\n".join(abstraction_info_for_prompt), project_name, shared, language # Return language

    def exec(self, prep_res):
        context, abstraction_listing, project_name, shared, language = prep_res  # Unpack project name and language
...
class OrderChapters(Node):
    def prep(self, shared):
        ...
        for rel in relationships['details']:
            try:
                from_name = abstractions[rel['from']]['name'] if 0 <= rel['from'] < len(abstractions) else "Unknown"
                to_name = abstractions[rel['to']]['name'] if 0 <= rel['to'] < len(abstractions) else "Unknown"
                # Use potentially translated 'label'
                context += f"- From {rel['from']} ({from_name}) to {rel['to']} ({to_name}): {rel['label']}\n" # Label might be translated
            except (IndexError, KeyError, TypeError) as e:
                print(f"Warning: Error processing relationship {rel}: {str(e)}")
                context += f"- From {rel.get('from', '?')} to {rel.get('to', '?')}: {rel.get('label', 'Unknown')}\n"

        list_lang_note = ""
        if language.lower() != "english":
             list_lang_note = f" (Names might be in {language.capitalize()})"

        return abstraction_listing, context, len(abstractions), project_name, shared, list_lang_note

    def exec(self, prep_res):
        abstraction_listing, context, num_abstractions, project_name, shared, list_lang_note = prep_res
...
"""
        chapter_content = call_llm_with_settings(prompt, self.shared)
